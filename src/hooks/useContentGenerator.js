...
import { generateBrandContent } from '../api/contentGenerator';
import { calculateBrandVoiceScore, analyzeBrandVoiceAlignment } from '../utils/brandVoiceScorer';

/**
 * Custom hook for managing content generation state and operations
...
export function useContentGenerator() {
  ...
  /**
   * Generate brand content for all platforms
  ...
  const generate = useCallback(async (brandProfile, contentBrief, options = {}) => {
    ...
    try {
      ...
      // Generate content
      const generatedContent = await generateBrandContent(brandProfile, contentBrief);
  ...
  const processResultsWithScoring = async (
    ...
    for (let i = 0; i < platforms.length; i++) {
      ...
      if (onProgress) {
        onProgress({
          platform,
          progress: (i / platforms.length) * 100,
          completed: i,
      ...
      if (onPlatformComplete) {
        onPlatformComplete({
          ...
          progress: ((i + 1) / platforms.length) * 100
        });
      }
    }
  ...
  const generateForPlatform = useCallback(async (platform, brandProfile, contentBrief, options = {}) => {
    ...
    try {
      ...
      // Create a modified content brief for single platform
      const singlePlatformBrief = {
        ...contentBrief,
        platform
      };

      const generatedContent = await generateBrandContent(brandProfile, singlePlatformBrief);
